...
**The World English Bible is in the open-access set** — *believed, not confirmed: no live key has been used to check, so verify it against your own key before you rely on the default* — so it resolves on a new key
